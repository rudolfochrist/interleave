...
To keep a multi-PDF notes file add a new headline for this PDF and set the =INTERLEAVE_PDF= property on it.
...
*Notice that you have to decide. It is not possible to keep a multi-PDF and normal notes in the same file.*
